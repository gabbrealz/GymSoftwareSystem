...
const AddLog = ({ isOpen, onClose, onAdd }) => {
    ...
    const handleSubmit = (e) => {
        ...
        onAdd({ ...formData, timestamp });
        setFormData({ id: '', name: '', memberId: '' });
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="rounded-[10px] shadow-2xl w-full max-w-md mx-4 border border-white/10 overflow-hidden bg-[#1a1a1a]">
                <div className="px-6 py-5 flex justify-between items-center border-b border-white/5 bg-[#770e00]/80">
                    <h2 className="text-xl font-bold text-white tracking-wide">Log In</h2>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-4">
                    {[
                        { label: 'Log ID', name: 'id', type: 'text', placeholder: 'e.g. 001' },
                        { label: 'Full Name', name: 'name', type: 'text', placeholder: 'Enter full name' },
                        { label: 'Member ID', name: 'memberId', type: 'text', placeholder: 'e.g. 12345 (Optional)', required: false },
                    ].map((field) => (
                        <div key={field.name}>
                            <label className="block text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1.5">{field.label}</label>
                            <input
                                type={field.type}
                                name={field.name}
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <button type="submit" className="flex-1 py-2 rounded-[8px] text-white font-bold bg-[#770e00] shadow-lg hover:scale-[1.02] transition-all">Log In</button>
                    </div>
                </form>
            </div>
        </div>
